/**
  ...
  ******************************************************************************
	...
	*PWM频率21kHz,可用脚0,3,4,5,6
	*LED2,LED3分别为PWM 5,6
	
	*ADC可用脚0,1,2,3,4,5,6,7
	*板载电位器为 8
	
	***Cube重新生成之后需要修改：
	1.sd_diskio.c里BSP_SD_ReadBlocks->BSP_SD_ReadBlocks_DMA
				   BSP_SD_WriteBlocks->BSP_SD_WriteBlocks_DMA
	2.如果使用了micros()函数则需要修改
	HAL_SYSTICK_Config(HAL_RCC_GetHCLKFreq() / 1000)为
...
/* USER CODE BEGIN PV */
/* Private variables ---------------------------------------------------------*/
FATFS SDFatFs;              //FAT文件系统对象
FIL MyFile;                 //文件对象
extern char SD_Path[4];     //SD卡逻辑路径
FRESULT res;                //SD卡操作结果
uint32_t byteswritten, bytesread; //成功写入/读取的字节数
char wtext[100];    //写入数据buf
char rtext[100];         //读取数据buf

MPU6050 mpu6050;

extern uint16_t ADCConvertedValue[9];  //ADC采样值
int i;
/* USER CODE END PV */
